...
Therefore, in vitro experiments, while useful to isolate and dissect particular aspects of a biological system, can give highly artificial results as the protein or reaction is not examined in its native environment. 
...
To this effect, a variety of systems exist (i.e. CRISPR/Cas9 knock-in/-out, RNAi-mediated knockdown, plasmid-based transient and constitutive overexpression, lentiviral transduction etc) and their advantages, caveats and suitability for the model system at hand should be carefully considered. 
For instance, should the POIs be encoded by an essential gene, by definition, a complete and stable knockout might not be achievable due to their critical functions maintaining cellular viability and proliferation ([@DOI: 10.1093/nar/gkt1131] for an overview of essential genes across multiple kingdoms of life, @DOI: 10.1016/j.cell.2022.10.017). 
In these extreme cases, attenuation of expression (i.e. using RNA interference (RNAi) or controlled degradation (see below)) rather than complete repression of a GOI can be used to probe for protein function. 
...
It is not always possible to fully avoid these artifacts, but a number of strategies exist to mitigate them.
Extensive validation of any overexpressed GOI is critical to ensure it closely reflects the functions of its endogenous counterpart and might involve assessing protein localisation (i.e. by imaging techniques such as microscopy and flow cytometry), protein abundance (i.e. by mass spectrometry or immunoblot analysis (see below)) and phenotypic assays, where applicable and possible. 

### Functional genomics techniques for MS validation

Typical follow-up experiments to validate mass-spectrometry derived insights often involve the acute depletion POIs to monitor cellular phenotypes. 
Here we discuss a selection of techniques to effectively modulate gene expression and important considerations when planning functional genomics experiments for target validation. 
...
However, especially CRISPR/Cas-mediated gene deletion is a permanent genetic modification and is not free from off-target effects ([@DOI: 10.3390/cells9071608] for a detailed discussion on how to minimise these off-target effects). 
...
When generating stable expression cell lines, being able to precisely control the genomic integration site of the transgene reduces genetic heterogeneity and limits potential off-targeting effects – both are important considerations in validation experiments. 
...
Site-directed isogenic integration of any GOI at the FRT site, which is under a tetracycline-inducible promoter and a hygromycin resistance gene, allows for facile generation of tetracycline/doxycycline-inducible isogeneic expression cell lines (for an example, see [@DOI: 10.1007/978-1-61779-126-0_2 ]. 
