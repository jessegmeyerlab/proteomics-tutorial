...
To investigate the biological function of a protein or pathway, direct genetic manipulation of the biological system at hand (e.g. modulating the expression of a POI by overexpression or knockout-/down experiments) can be minimally invasive when performed correctly and preserve the system in a near-native state.
To this purpose, a variety of powerful and streamlined technologies exist (i.e. CRISPR/Cas-mediated knock-in/-out, RNAi-mediated knockdown, plasmid-based transient and constitutive overexpression etc) and their advantages, caveats and suitability for the model system at hand should be carefully considered. 
For instance, should the POI be encoded by an essential gene, by definition, a complete and stable knockout might not be advisable due to it's critical role in maintaining cellular viability and proliferation ([@DOI: 10.1093/nar/gkt1131] for an overview of essential genes across multiple kingdoms of life, @DOI: 10.1016/j.cell.2022.10.017). 
In these extreme cases, attenuated expression (i.e. using RNA interference (RNAi) or controlled degradation (see below)) rather than complete repression of a GOI can be used to probe for protein function. 
...
It is not always possible to fully avoid these expression artifacts, but a number of mitigation strategies (i.e. inducible expression) will be discussed below.
...
Since each one of these technologies comes with their own unique advantages and caveats, the approach taken is highly dependent on the biological question under investigation.
...
When expressed in mammalian cells, the bacterially-derived Cas9 ndonuclease can be guided with the help of a short guide RNA (gRNA) to a genomic location of interest, where it then introduces a DNA double strand break in a highly controlled manner (for a detailed discussion see [@DOI: 10.1007/s00294-019-01040-3]).
The cell's DNA double-stand break repair machinery then intruduces base pair insertions or deletions (indels) via non-homologous-end-joining (NHEJ), causing missense mutations, frameshift mutations and/or the introduction of premaure stop codons, leading to premature termination of gene expression or non-functional gene products.
Similarly, concomitant provision of a complementary DNA donor template encoding a desired gene modification (i.e. insertion of a stretch of DNA) will trigger homology-directed repair (HDR), effectively resulting in gene knockin or base editing [@DOI: 10.1007/s00294-019-01040-3].     
