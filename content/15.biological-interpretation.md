...
In this section, we aim to present a very concise overview of how proteomic data can be effectively contextualised, orthogonally validated and used to generate new hypotheses.
The computational workflows to interpret mass spectrometry data as described above are sophisticated, powerful tools, but also show important limitations and caveats due to their dependence on limited prior knowledge, specific experimental parameters or data quality restraints (see section ‘Analysis of Raw Data’). 
These inherent biases can give rise to ambiguous or spurious interpretation of the raw data even when these workflows are applied correctly and to the best of the experimenter’s knowledge.
Therefore, researchers will oftentimes be asked by scientific journals to provide independent orthogonal validation of their proteomics data and not performing orthogonal validation can be a major roadblock in the publication process. 

The aim of validating data obtained by proteomics approaches should always be two-fold by demonstrating that the conclusions arrived at by proteomics data acquisition and analysis are valid and relevant.
Depending on the question at hand, researchers can draw on a myriad of techniques to validate MS-derived hypotheses in appropriate cellular, organismal or in vitro models.
The following paragraphs represent only a high-level, stringent, non-exhaustive selection of orthogonal validation approaches and aims to emphasise the importance of implementing assays that challenge assumptions gained from proteomics data analysis pipelines.
...
Given these challenges, the method(s) of choice warrants careful consideration. The choice of the orthogonal validation method used is highly context-dependent.
...
To assess the success of any experiment, positive controls are invaluable. 
Internal positive controls can be proteins whose behaviour under the experimental conditions applied can be deduced a priory from prior knowledge (i.e. the scientific literature or public databases [see section ‘Databases’]). 
Candidates representing abundant proteins that show high sequence coverage and are detected with high confidence might not necessarily need extensive orthogonal validation when compared with proteins of intermediate to low abundance that might be more challenging to faithfully quantify by proteomics alone (i.e. many membrane proteins or transcription factors). 
Similarly, since the proteome is rarely comprehensively quantified in any single proteomics experiment, POIs that are critical for an observed biological change might not be part of the dataset. 
In these cases, additional, targeted analyses drawing on the vast repertoire of cell biology and biochemistry techniques might help to support or discredit proteomics-based hypotheses. 

### General considerations 

Once one or multiple POIs have been selected based on the parameters set in a proteomics experiment (i.e. (adjusted) p value and/or fold change), orthogonal validation experiments should, as far as possible, be conducted under physiologically relevant conditions to mitigate artifical outcomes. 
Therefore, in vitro experiments, while useful to isolate and dissect particular aspects of a biological system, can give highly artificial results as the protein or reaction is removed from its native environment. 
To investigate the function of a protein or pathway in its native environment, researchers often resort to direct genetic manipulation of the biological system at hand.
This generally takes the form of modulating the expression of a POI by overexpression or knockout-/down experiments. 
To this effect, a variety of systems exist (i.e. CRISPR/Cas9 knock-in/-out, RNAi-mediated knockdown, plasmid-based transient and constitutive overexpression, lentiviral transduction etc) and their advantages and caveats should be carefully considered. 
For instance, should the POIs be encoded by an essential gene, by definition, a complete and stable knockout might not be achievable due to their essential roles in cellular viability and proliferation ([@DOI: 10.1093/nar/gkt1131] for an overview of essential genes across multiple kingdoms of life, @DOI: 10.1016/j.cell.2022.10.017). 
In these extreme cases, attenuation of expression (i.e. using RNAi or induced degradation (see below)) rather than complete repression of a GOI can be used to probe for protein function. Epitope tagging and/ or exogenous expression of a GOI can be a powerful approach in assessing PPIs and investigating proteins of low abundance. 
However, overexpression artifacts are common [@DOI: 10.1016/j.mrrev.2017.05.002]. 

It is not always possible to fully avoid these artifacts, but a number of strategies exist to mitigate such. Extensive validation of any overexpressed GOI is critical to ensure it closely reflects the functions of its endogenous counterpart and might involve assessing protein localisation (i.e. by imaging techniques such as microscopy and flow cytometry), protein abundance (i.e. by mass spectrometry or immunoblot analysis (see below)) and phenotypic assays, where applicable and possible. 
Validation techniques are as manifold as biological questions and discussions thereof may easily fill multiple textbooks. 
The following sections are meant to paint with a broad brush stroke a picture of useful methodolgies with which to validate and follow up MS-data derived observations. 
...
### Functional genomics techniques for MS validation

Typical follow-up experiments to validate mass-spectrometry derived insights often involve the acute depletion or knockout of POIs to monitor cellular phenotypes. 
We will therefore give a high-level overview of techniques to effectively modulate gene expression and important considerations when planning functional genomics experiments for target validation. 
...
Gene deletion or knockdown, prevention of expressing a functional protein, in particular, is a powerful means to interrogate the role of one or more proteins in the phenotypes under investigation. 
To this end, well-established technologies deserving mention are RNA interference (RNAi) in the form of CRISPR/Cas9-or TALEN-mediated gene knockout and siRNA/shRNA- or miRNA-mediated gene knockdown [@DOI: 10.1016/j.molcel.2015.04.028]. 
Since each one of these technologies comes with certain advantages and caveats, the gene editing approach taken is highly dependent on the biological question under investigation. 
CRISPR/Cas-based gene deletion technologies allow for targeting of individual genes with relative ease and high efficiency and specificity. 

However, especially CRISPR/Cas-mediated gene deletion is a permanent genetic modification and is not free of off-target effects. 
Moreover, long-term depletion (or upregulation) of a POI can in some cases have dramatic systemic consequences and constitute an acute selection pressure leading to compensatory stress-induced adaptation that might obfuscate primary loss-of-function phenotypes and pose a substantial hurdle to the interpretability of biological data. 
...
siRNA-mediated knockdown by transient transfection is typically achieved at shorter time frames (24 – 96h), depending on the turnover of the POI. 
...
Multiple eukaryotic and prokaryotic inducible expression systems have been developed that allow for the controlled biosynthesis or depletion of one or more POIs.
Amongst these, a popular and dependable choice for mammalian cells are tetracycline-controlled operon systems, which allow up- or downregulation of a GOI in the presence of the antibiotic tetracycline or its derivative doxycycline. 
...
The TetO binds a co-expressed Tet-repressor protein blocking transcription. 
In the presence of tetracycline, which is added to the cells, the repressor dissociates from the operon, thus de-repressing the GOI. Different configurations of this potent system exist, allowing for more flexibility in experimental design. 
In the Tet-OFF system, the Tet repressor is fused to a eukaryotic transactivator (the chimeric fusion construct is termed tTA) and addition of tetracycline abolishes TetO binding and thus suppresses transcriptional activation. 
...
When generating stable expression cell lines, being able to precisely control the genomic integration site of the transgene reduces genetic heterogeneity and limits potential off-targeting effects – important considerations in validation experiments. 
This ability is realised in the FlpIn-T-REx technology which harnesses Flp-recombinase mediated DNA recombination at a strictly defined genomic locus (the FRT site). 
Site-directed integration of any GOI under a tetracycline-inducible promoter and a hygromycin resistance gene allows for facile generation of tetracycline/doxycycline-inducible isogeneic expression cell lines. 

### Validation and interpretation of protein abundance changes

To validate protein abundance changes observed by quantitative bottom-up proteomics or simply assess the success of functional genetic manipulation as part of an orthogonal follow-up experiment (see above), the experimenter typically resorts to antibody-based techniques such as immunoblotting analysis or immunofluorescence and immunohistological imaging of a POI. 
...
In this case, more accurate orthogonal quantitation of proteins might be achieved by stable isotope labelling (SILAC/TMT/iTRAQ) and/or SRM/PRM (see sections INSERT SECTION TITLE HERE). 
SDS-PAGE and immunoblot analysis is powerful and facile tool to quickly validate protein abundance changes. 
However, short of introducing epitope tags to the endogenous POI, this technique is contingent on the availability of specific antibodies, which can present a problem when investigating poorly characterised proteins or working with model organisms for which the commercial availability of specific antibodies is limited (this is particularly problematic for ‘unconventional’ model organisms). 
...
The abundance of a protein in a complex sample directly reflects its intrinsic stability or expression levels under the conditions of interest. 
...
Radioisotope labelling can be a powerful and accurate way to monitor protein turnover. 
...
The type of radiolabelling described above is relatively labour-intense, low-throughput and has the obvious disadvantage of requiring radioactive material, which needs to be handled under strict safety precautions. 
...
Briefly, gene expression changes can be determined with high fidelity using quantitative real-time PCR (qRT-PCR) or RNA-Seq can measure changes in gene transcription or mRNA turnover (for an extensive discussion of both technologies, please see [@DOI: 10.21775/cimb.016.001] and [@DOI: 10.1038/nrg2484], respectively). 
...
When these split reporters are fused to two interacting proteins (so-called bait and prey proteins), the binding of bait to prey induces the spatial restriction needed to fully complement the reporter. Commonly used reporter complementation systems are split fluorescent proteins (i.e. GFP, YFP) [@DOI: 10.3390/ijms20143479], ubiquitin [@DOI: 10.1093/bfgp/1.3.230], luciferase [@DOI: ], TEV protease [@DOI: ], beta-lactamase [@DOI: ], beta-galactosidase [@DOI: ], Gal4 [@DOI: ] or DHFR [@DOI: ]. 
The resulting functional readout of these complementation system depends on which split reporter is used. 
In general, the split luciferase shows enhanced sensitivity over fluorescence-based systems as background luminescence is low. 
Two-hybrid assays are based on a similar functional complementation strategy as fragment complementation systems. 
Conventionally, two self-complementing transcription factor fragments are fused bait and prey proteins, respectively, leading to the restoration of a functional transcription factor only upon prey-bait interaction. 
...
The yeast-2-hybrid system (Y2H) is deserving of mention here as it had been the very first 2-hybrid system established in 1989 [@DOI: 10.1038/340245a0] and has ever since proven to be extremely versatile (multiple auxotrophic reporters and markers of phenotypic sensitivity available), cheap, lends itself to high-throughput screens and variants have been developed that allow for the investigation of membrane-protein interactions (i.e. membrane Y2H) [@DOI: 10.3390/ijms10062763], [@DOI: 10.4149/gpb_2021035]. 
Despite the many advantages of the Y2H, critical drawbacks include the potential of misfolding of bait and prey when fused to a complementation reporter, expression at non-physiological levels, the lack of control over posttranslational modifications that might be important for the PPI under investigation, and the potential requirement of kingdom- or species specific folding factors for the bait/prey under investigation (i.e. when probing PPI of mammalian proteins in Y2H). 
Principles of the Y2H have also been adapted to mammalian systems, which circumvent some of the aforementioned drawbacks of Y2H [@DOI: 10.1016/j.drudis.2020.01.022]. 
...
AP typically relies on the isolation of a transgenic POI by its epitope tag (using epitope-specific matrix-conjugated proteins (antibodies or epitope-binding proteins )), while Co-IP harnesses specific antibodies directly targeting the POI. 
...
AP is not solely restricted to PPIs, but can also be adapted to protein interactions with other biomolecules such as RNA [@DOI: 10.3390/ijms160922456]. 
...
These DNA products can then be visualised by in situ hybridisation with fluorescently labelled oligonucleotides ([@DOI: 10.1007/82_2013_334] for a detailed discussion). 
